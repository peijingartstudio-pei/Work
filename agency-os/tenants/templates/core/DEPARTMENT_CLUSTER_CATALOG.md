...
> **Owner**：本檔為 **「客戶要啟用哪些企業能力」** 的**選型正本**；**填檔路由**仍以 **`DEPARTMENT_COVERAGE_MATRIX.md`** 的列為準（15 列 **交付歸桶** ≠ 客戶 org chart 線條數）。  
> **原則**：不是每家都要 17+ 格；**開案勾選子集**，**日後**用同一套 ID **擴充**（更新選擇檔 + 矩陣 Notes + 主檔 + 合約範圍）。
...
## 能力模組總覽（對照營收／平台／供應鏈／財務風控／企業支援／國際化／進階）

以下 **17+2** 項為 **選型單位**；**`smb_default`**：若無特別說明，SMB 常 **off**（以你報價為準，此欄僅供內部參考）。
...
| `CLU_SALES_BD` | 銷售／商務拓展 | 營收 | `product_digital` + `strategy_gov`（Notes 註 BD／招商） | off |
...
| `CLU_DATA_AI` | 數據／BI／AI／推薦 | 平台 | `data_privacy` + `engineering` | off |
| `CLU_MERCHANDISING` | 採購／商品／選品定價 | 供應鏈 | `procurement` + `finance_ma`（Notes） | 視業態 |
| `CLU_LOGISTICS` | 物流／倉儲／履約 | 供應鏈 | `operations_delivery` + `tax_customs`（跨境時） | off |
| `CLU_ORDER_OPS` | 訂單／退貨／第一線客服營運 | 供應鏈 | `operations_delivery` | on |
| `CLU_FINANCE` | 財務／管理會計 | 財務 | `finance_ma` | on |
| `CLU_RISK_FRAUD` | 風控／反詐欺 | 財務 | `risk_security` + `finance_ma`（Notes） | off |
| `CLU_TAX_TRADE` | 稅務／關務（資訊橋） | 財務 | `tax_customs` | off |
| `CLU_LEGAL` | 法務／合規 | 企業支援 | `legal` | off |
| `CLU_HR` | 人力資源（交付觸及時） | 企業支援 | `hr` | off |
| `CLU_ADMIN_GA` | 行政／總務 | 企業支援 | `procurement`（Notes：非商品採購／內控） | off |
...
| `CLU_IR` | 投資人關係 | 橫向 | `ir` | off |
| `CLU_ECOSYSTEM` | 平台生態（廣告／金融／SaaS 給商家） | **進階** | `product_digital` + `legal` + `finance_ma`（Notes） | off |
| `CLU_CONTENT_MEDIA` | 內容／媒體／直播 | **進階** | `marketing_growth` + `brand_comms` | off |
